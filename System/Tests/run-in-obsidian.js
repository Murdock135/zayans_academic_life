...
module.exports = async ({app, quickAddApi, obsidian}) => {
 ...
 try {
  ...
  let homeEl;
  await test('Home rendering: standalone and active-project tasks render in separate sections',async()=>{homeEl=await render(home);await wait(()=>tasksIn(homeEl).some(el=>el.textContent.includes('ACTIVE_TASK'))&&nextTasksIn(homeEl).some(el=>el.textContent.includes('GLOBAL_TASK')),'both Home task sections');const projectText=tasksIn(homeEl).map(el=>el.textContent).join('\n'),nextText=nextTasksIn(homeEl).map(el=>el.textContent).join('\n');assert(projectText.includes('SCRATCH_TASK')&&!projectText.includes('PAUSED_TASK')&&!projectText.includes('COMPLETED_TASK'),'Incorrect project filtering: '+projectText);assert(!nextText.includes('GLOBAL_COMPLETED')&&!nextText.includes('INBOX_TASK')&&!nextText.includes('SIMILAR_NAME_TASK'),'Incorrect standalone filtering: '+nextText);assert(!projectText.includes('GLOBAL_TASK')&&!nextText.includes('ACTIVE_TASK'),'Task sections overlap');noErrors(homeEl)});
  ...
  await test('Home interaction: changing project status removes only project tasks',async()=>{await set(project,{status:['archived']});const el=await render(home);await wait(()=>el.textContent.includes('No open tasks from active projects.')&&nextTasksIn(el).some(row=>row.textContent.includes('GLOBAL_REMAINS')),'status-independent standalone task');assert(tasksIn(el).length===0,'Archived tasks remain visible');noErrors(el);await set(project,{status:['active']})});
